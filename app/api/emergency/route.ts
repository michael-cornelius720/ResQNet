...
export async function POST(request: NextRequest) {
  try {
    ...
    let notifiedHospitals: { id: string; name: string; distance: number }[] = [];
    ...
    if (selected_hospital_id) {
      ...
      // Calculate bounding box for initial filtering to avoid fetching all hospitals (and hitting 1000 row limit)
      // 1 deg lat ~= 111km
      // 1 deg lon ~= 111km * cos(lat)
      const latDelta = (radius_km + 5) / 111; // Add 5km buffer
      const lonDelta = (radius_km + 5) / (111 * Math.cos(latitude * Math.PI / 180));
