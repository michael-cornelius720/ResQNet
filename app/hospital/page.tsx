...
export default function HospitalDashboard() {
  ...
  // Helper for fuzzy matching hospital names
  const areHospitalNamesSimilar = (name1: string, name2: string) => {
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    try {
      // Fetch ALL pending appointments, then filter client-side for better matching
      ...
      if (data.success) {
        ...
        const myAppointments = data.data.filter((appt: Appointment) =>
          areHospitalNamesSimilar(appt.hospital_name, hospitalInfo.name)
        );
